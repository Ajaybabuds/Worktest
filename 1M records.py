...
def top_most(dataframe,field,val,grp,met):
    return dataframe.loc[(dataframe[field]==val)].groupby([grp]).agg({met: lambda x:x.describe(include='object')['top']})
def convert(df):
...
#data=pd.read_csv(r"C:\Users\ajay\Downloads\Jonathan.csv",quotechar='"',encoding='utf-8',low_memory=False)
#data=data.iloc[:,1:]
client=pymongo.MongoClient("mongodb+srv://rugstore:netscore@analytic.k0yws.mongodb.net/test")


def get_data(db,cn):
    db_connect= client[db]
    collection = db_connect[cn]
    d=list(collection.find())# make hashable for st.cache
    data=pd.DataFrame(d)
    return data

data=get_data('Netdbb','Jonathany')
...
report = st.sidebar.selectbox("Select a Reports", options=['Sale Report', 'Customer Report', 'Item Report', 'Compare',
                                                           "Multi-dimensional", 'Top Most', 'Classification','Forecast',
                                                           'RFM'])
if 'Sale Report' in report:
    ...
    metric = st.sidebar.selectbox("Select a field",
                                  options=sorted(data.select_dtypes(exclude=['datetime64[ns]', 'int64']).columns,reverse=True))
...
elif "Classification" in report:
    bulb = pd.read_csv(r'C:\Users\ajay\Bulb.csv')
    bulb = bulb.iloc[:, 1:]
    col1, col2, col3 = st.columns(3)
    with col1:
        clr = st.text_input("Pick a Color")
    with col2:
        clr1 = st.text_input("Pick another color")
    with col3:
        shade = st.text_input("Choose a color shade")
    col4, col5, col6 = st.columns(3)
    with col4:
        ln = st.number_input("Enter a length of bulb", min_value=25)
    with col5:
        wd = st.number_input("Enter a width of bulb", min_value=25)
    with col6:
        dm = st.number_input("Enter a diameter of bulb", min_value=25)
    ohe = OneHotEncoder(handle_unknown='ignore').fit(bulb.loc[:, ['Color_1', 'Color_2', 'Color_Shade']])
    oe = OrdinalEncoder().fit(bulb['Class (no hierarchy)'].to_numpy()[:, np.newaxis])
    model = pickle.load(open("C:\\Users\\ajay\\Bulbclassification.sav", 'rb'))
    samples = np.hstack((ohe.transform([[clr, clr1, shade]]).toarray(), np.array([[ln, wd, dm]])))
    if st.button("Classification of Bulb"):
        st.write(oe.inverse_transform(model.predict(samples)[:, np.newaxis]))
    ...
    st.subheader("RFM analysis is a marketing technique used to quantitatively rank and group customers based on the recency, frequency and monetary total of their recent transactions to identify the best customers and perform targeted marketing campaigns.")
    tm=pd.to_datetime(st.sidebar.date_input("Select Date"))
    field=st.sidebar.selectbox("Select a Field",options=data.select_dtypes(exclude=['float64', 'datetime64[ns]','int64']).columns)
